...
import org.motechproject.alerts.EventKeys;
...
public class AlertEventHandlerTest {
    ...
    public static final long ALERT_ID = 123;
    ...
    public void shouldChangeStatusForClose() {
        ...
        assertEquals(AlertStatus.CLOSED, captor.getValue().getAll().get(UpdateCriterion.STATUS));
    }
    ...
    public void shouldChangeStatusForRead() {
        ...
        alertEventHandler.updateStatus(event);
        verify(alertService).update(eq(ALERT_ID), captor.capture());
        assertEquals(AlertStatus.READ, captor.getValue().getAll().get(UpdateCriterion.STATUS));
    }
}
