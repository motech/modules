...
public class CourseStructureServiceIT extends BasePaxIT {
    ...
    //
    // Unused units:
    // chapter_3, chapter_4, chapter_5, chapter_6, lesson_6, lesson_7, lesson_8,
    // lesson_9, quiz_1, quiz_4, quiz_5, quiz_6
    @Test
    public void updateCourseStructureTest1() throws Exception {
        ...
        assertEquals(asList(LESSON_1, LESSON_2, LESSON_4), lessons);
        List<CourseUnitState> states = extract(chapter.getLessons(), on(Lesson.class).getState());
        assertEquals(asList(CourseUnitState.Pending, CourseUnitState.Pending, CourseUnitState.Active), states);

        chapter = course.getChapters().get(1);
        assertNotNull(chapter.getQuiz());
        lessons = extract(chapter.getLessons(), on(Lesson.class).getName());
        assertEquals(asList(LESSON_3), lessons);
        states = extract(chapter.getLessons(), on(Lesson.class).getState());
        assertEquals(asList(CourseUnitState.Active), states);
        ...
        assertEquals(asList(QUIZ_1, QUIZ_4, QUIZ_5, QUIZ_6), unusedUnits);

        unusedUnits = extract(unusedLessons, on(Lesson.class).getName());
        Collections.sort(unusedUnits);
        assertEquals(asList(LESSON_6, LESSON_7, LESSON_8, LESSON_9), unusedUnits);

        unusedUnits = extract(unusedChapters, on(Chapter.class).getName());
        Collections.sort(unusedUnits);
        assertEquals(asList(CHAPTER_3, CHAPTER_4, CHAPTER_5, CHAPTER_6), unusedUnits);
    }
