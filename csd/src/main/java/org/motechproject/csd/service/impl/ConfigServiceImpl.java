...
public class ConfigServiceImpl implements ConfigService {
    ...
    public Config getConfig(String xmlUrl) {
        ...
        throw new ConfigNotFoundException(String.format("Unknown config: '%s'.", xmlUrl));
    }
    ...
    private void schedule(Config config, Integer key) {
        ...
        if (startDate != null && !startDate.isEmpty()) {
            startDateTime = DateTime.parse(startDate, DateTimeFormat.forPattern(Config.DATE_TIME_PICKER_FORMAT));
        }

        csdScheduler.scheduleXmlConsumerRepeatingJob(eventParameters, startDateTime, null, period, key.toString());
    }
}
