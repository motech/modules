...
public class AllAdherenceLogsIT extends SpringIntegrationTest {
    ...
    public void shouldSaveAdherenceLog() {
        AdherenceLog adherenceLog = new AdherenceLog("externalId", "treatmentId", DateUtil.now());
    ...
    public void shouldFetchAdherenceLogByKey() {
        ...
        AdherenceLog toBeFound = new AdherenceLog("externalId", "treatmentId", now);
        AdherenceLog toBeIgnoredByExternalId = new AdherenceLog("otherExternalId", "treatmentId", now);
        AdherenceLog toBeIgnoredByTreatmentId = new AdherenceLog("externalId", "otherTreatmentId", now);
        AdherenceLog toBeIgnoredByDate = new AdherenceLog("externalId", "treatmentId", now.plusDays(1));

        addAll(toBeFound, toBeIgnoredByExternalId, toBeIgnoredByTreatmentId, toBeIgnoredByDate);

        assertArrayEquals(
                new AdherenceLog[]{toBeFound},
                allDosageLogs.findLogsBy("externalId", "treatmentId", now).toArray()
        );
    }
    ...
    public void shouldFetchAllAdherenceLogsByKeyTillKeyDate() {
        ...
        AdherenceLog toBeFound = new AdherenceLog("externalId", "treatmentId", now);
        AdherenceLog anotherToBeFound = new AdherenceLog("externalId", "treatmentId", now.plusDays(1));
        AdherenceLog toBeIgnoredByExternalId = new AdherenceLog("otherExternalId", "treatmentId", now);
        AdherenceLog toBeIgnoredByTreatmentId = new AdherenceLog("externalId", "otherTreatmentId", now);

        addAll(toBeFound, anotherToBeFound, toBeIgnoredByExternalId, toBeIgnoredByTreatmentId);

        DateTime asOf = now.plusDays(5);
    ...
    public void shouldNotFetchAllAdherenceLogsByKeyBeyondKeyDate() {
        ...
        AdherenceLog hasDateWithinKeyDate = new AdherenceLog("externalId", "treatmentId", now);
        AdherenceLog hasDateBeyondKeyDate = new AdherenceLog("externalId", "treatmentId", now.plusDays(1));
